...
class PascalVOCDetectionEvaluator(DatasetEvaluator):
    def __init__(self, dataset_name, cfg=None):
        ...
        if cfg is not None:
            ...
            if 'RetinaNet' in cfg.MODEL.META_ARCHITECTURE:
                self.total_num_class = cfg.MODEL.RETINANET.NUM_CLASSES
                self.unknown_class_index = self.total_num_class - 1
                self.num_known_classes = cfg.MODEL.RETINANET.NUM_KNOWN_CLASSES
                self.known_classes = self._class_names[:self.num_known_classes]
                self.num_base_classes = cfg.MODEL.RETINANET.NUM_BASE_CLASSES
                self.base_classes = self._class_names[:self.num_base_classes]
                self.novel_classes = self._class_names[self.num_base_classes:self.num_known_classes]
            else:
    ...
    def evaluate(self):
        ...
        #self.output_dir = "/home/subinyi/Users/FSOSOD/DeFRCN-main/output/voc/split_1"
        ...
        for cls_id, cls_name in enumerate(self._class_names):
            ...
            for thresh in range(50, 100, 5):
                ...
                if cls_name == 'unknown':
                    plt.title(label="AP", fontsize=14)
                    plt.xlabel("recall", fontsize=16)
                    plt.ylabel("precision", fontsize=16)
                    plt.plot(rec, prec, 'dodgerblue',linewidth=2.0, label="$AP_{unknown}$")
                    #plt.savefig('/home/subinyi/Users/FSOSOD/DeFRCN-main/PR_curves/'+str(ap)+'.svg', format='svg', bbox_inches="tight")
                    plt.savefig('PR_curves/' + str(ap) + '.svg', format='svg',
                                bbox_inches="tight")

                    # plt.show()

                if self.base_classes is not None and cls_name in self.base_classes:
                    aps_base[thresh].append(ap*100)
                    exist_base = True


                if self.novel_classes is not None and cls_name in self.novel_classes:
        ...
        if exist_base:
            results_2d.update({
                "AP@B": np.mean(aps[50][:self.num_base_classes]),
                "P@B": np.mean(precs[50][:self.num_base_classes]),
                "R@B": np.mean(recs[50][:self.num_base_classes])
            })
        if exist_novel:
            results_2d.update({
                "AP@N": np.mean(aps[50][self.num_base_classes:self.num_known_classes]),
                "P@N": np.mean(precs[50][self.num_base_classes:self.num_known_classes]),
                "R@N": np.mean(recs[50][self.num_base_classes:self.num_known_classes])
            })
        ...
        for i in range(50, 100, 5):
            aa.append(np.mean(recs[i][-1]))
        results_2d.update({
            "AR@U": np.mean(aa),
        })

        savepath = os.path.join(self.output_dir, "AR.txt")
