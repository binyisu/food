...
class ELoss(nn.Module):
    ...
    def _soft_cross_entropy(self, input_gt_scores: Tensor, input: Tensor, target: Tensor):
    ...
    def _sampling(self, scores: Tensor, labels: Tensor):
        ...
        # max unknown prob
            ...
            pos_metric = 22 / torch.sum(torch.exp(fg_scores) + 1, dim=1)
            neg_metric = 22 / torch.sum(torch.exp(bg_scores) + 1, dim=1)
    ...
    def forward(self, scores: Tensor, labels: Tensor):
        ...
        # return self._soft_cross_entropy(A, mask_scores, targets.detach())
        return A_mean