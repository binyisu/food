...
def parse_args():
    ...
    parser.add_argument('--dataset', type=str, default='voc_coco', help='', choices=['voc_coco', 'voc'])
...
def main():
    ...
    if args.dataset in ['voc']:
        name_template = 'food_{}_r50_novelx_{}shot_seedx.yaml'
    ...
    elif args.dataset in ['voc_coco']:
        name_template = 'food_{}_r50_novel_{}shot_seedx.yaml'
        yaml_path = os.path.join(args.config_root, name_template.format(args.setting, args.shot))
        yaml_info = load_config_file(yaml_path)
        for i, lineinfo in enumerate(yaml_info):
            if '  TRAIN: ' in lineinfo:
                _str_ = '  TRAIN: ("voc_coco_trainval_{}_{}shot_seed{}", )\n'
